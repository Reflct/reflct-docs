...
For video, capture steady, continuous footage by physically moving the camera rather than rotating it. Start with wide arcs at a low height and progressively increase height while maintaining smooth motion. Include depth variations by changing your distance from the subject. Once wide shots are complete, move closer for detailed captures, ensuring all sides and angles are covered. Over-capture the scene—walls, ceilings, and floors—to allow flexibility during post-processing. Asymmetrical and diverse viewpoints are particularly effective for capturing intricate surfaces.

With video capture, you’ll need a process to extract the frames from the video as images. Depending on the length and frame rate of the video, capturing every frame could result in thousands of images. A good starting point is extracting three frames per second; if you were moving slowly during capture, one or two frames per second may suffice.

Once you have the frames extracted, isolate and remove any images with motion blur. See our GoPro guide for a detailed step-by-step process to maximize the quality of video captures, including tools to isolate clear frames.
