...
- **ISO:** Use a fixed value for ISO, ideally you should keep this low to minimize noise however good results can also be achieved with higher ISO values. Note that if you are training using the bilateral grid method then shooting with dynamic ISO is fine.
